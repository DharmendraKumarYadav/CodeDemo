...
@Component({
  selector: 'app-dealer-sale-bike-add-update',
  templateUrl: './dealer-sale-bike-add-update.component.html',
  styleUrls: ['./dealer-sale-bike-add-update.component.scss']
})
export class DealerSaleBikeAddUpdateComponent implements OnInit {
  formGroup: FormGroup;
  isLoading: boolean;
  ...
  constructor(private router: Router, private bikeSpecService: BikeSpecificationService, public dialogRef: MatDialogRef<DealerSaleBikeAddUpdateComponent>, @Inject(MAT_DIALOG_DATA) public modalData: ModalData, private accountService: AccountService, private bookingService: BookingService, private dialog: MatDialog, private bikeService: BikeService, private locationService: LocationService, private spinnerService: SpinnerService, private fb: FormBuilder, private showRoomService: ShowroomService, private notificationService: NotificationService) { }
  ...
  checkFormAction(data: ModalData) {
    if (data.action == UserAction.Edit) {
      this.isEdit = true;
      this.formName = "Edit Bike"
      console.log("EditModeData",data.data);
      this.populateFormData(data.data);
    } else if (data.action == UserAction.Add) {
      this.formName = "Create Bike"
      this.isAdd = true;
    }
    else if (data.action == UserAction.View) {
      this.formName = "View Bike"
      this.populateFormData(data.data);
      this.isView = true;
  ...
  initializeForm() {
    this.formGroup = this.fb.group({
      ...
      isTransferable:new FormControl(true),
    });
  }
  ...
  save() {
    ...
    this.showRoomService.createDealerBike(formValue).subscribe(
      (response: any) => {
        this.notificationService.showSuccess
          ("Bike created sucessfully.");
  ...
  update() {
    ...
    let spinerRef = this.spinnerService.start();
    this.showRoomService.updateDealerBike(formValue, formValue.id).subscribe(
      (response: any) => {
        this.notificationService.showSuccess
  ...
  onSelectBikeVariant($event) {
    let varaintData = $event.value;
  ...
  onSelectShowRoom($event) {
    let formValue = this.formGroup.value;
    let spinerRef = this.spinnerService.start();
    this.bikeService.getBikeVariantPriceByCity($event.value)
