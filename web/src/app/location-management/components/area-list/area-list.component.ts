...
export class AreaListComponent  implements OnInit {
  ...
  sortData(sortParameters: Sort) {
    ...
    if (sortParameters.direction === 'asc') {
      this.dataSorce = this.dataSorce.sort((a: Area, b: Area) => a[keyName].localeCompare(b[keyName]));
    } else if (sortParameters.direction === 'desc') {
      this.dataSorce = this.dataSorce.sort((a: Area, b: Area) => b[keyName].localeCompare(a[keyName]));
    } else {
  ...
  delete(rowData: any) {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult) {
        ...
        this.locationService.deleteArea(rowData.id).subscribe(result => {
          this.loadData();
          this.spinnerSerivce.stop();
          this.isLoading = false;
          this.notificationService.showSuccess
            ("Area deleted sucessfully.");
        });
      } else {
  ...
  loadData() {
    this.isLoading = false;
    this.locationService.getArea(-1,-1)
      .subscribe(results => {
        this.isLoading = true
