...
@Component({
  ...
  styleUrls: ['./body-style.component.scss']
})
export class BodyStyleComponent implements OnInit {
  formGroup: FormGroup;
  isLoading: boolean;
  formName: string = "Create BodyStyle"
  ...
  _showCaption = false;
  imageUrl:any;
  constructor(@Inject(MAT_DIALOG_DATA) public modalData: ModalData,private utilities:Utilities, private spinnerService: SpinnerService, private fb: FormBuilder, public dialogRef: MatDialogRef<BodyStyleComponent>, private bikeSpecService: BikeSpecificationService, private notificationService: NotificationService) { }
  ...
  save() {
    ...
    this.bikeSpecService.createBodyStyle(formValue).subscribe(
      (response: any) => {
        this.notificationService.showSuccess
          ("BodyStyle created sucessfully.");
  ...
  update() {
    ...
    this.bikeSpecService.updateBodyStyle(formValue,this.modalData.data.id).subscribe(
      (response: any) => {
        this.notificationService.showSuccess
          ("BodyStyle updated sucessfully.");
        this.isLoading = false;
        this.dialogRef.close();
        this.spinnerService.stop();
      },
