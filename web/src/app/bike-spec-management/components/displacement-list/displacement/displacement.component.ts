...
export class DisplacementComponent implements OnInit {
  formGroup: FormGroup;
  isLoading: boolean;
  formName: string = "Create Displacement"
  roles: Attribute[];
  isEdit: boolean = false;
  isAdd: boolean = false;
  isView: boolean = false;
  constructor(@Inject(MAT_DIALOG_DATA) public modalData: ModalData, private spinnerService: SpinnerService, private fb: FormBuilder, public dialogRef: MatDialogRef<DisplacementComponent>, private bikeSpecService: BikeSpecificationService, private notificationService: NotificationService) { }
  ...
  initializeForm() {
    this.formGroup = this.fb.group({
      ...
      value: new FormControl("", [Validators.required, NumericValidator]),
  ...
  save() {
    ...
    this.bikeSpecService.createDisplacement(formValue).subscribe(
      (response: any) => {
        this.notificationService.showSuccess
          ("Displacement created sucessfully.");
  ...
  update() {
    ...
    this.isLoading = true;
    let spinerRef = this.spinnerService.start();
    this.bikeSpecService.updateDisplacement(formValue, this.modalData.data.id).subscribe(
      (response: any) => {
        this.notificationService.showSuccess
          ("Displacement updated sucessfully.");
        this.isLoading = false;
        this.dialogRef.close();
        this.spinnerService.stop();
      },
